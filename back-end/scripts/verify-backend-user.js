...
async function verifyBackendUser() {
    ...
    try {
        ...
        if (users.length === 0) {
            ...
            console.log('\n⚠️  You need to create this user first.');
        ...
        try {
            ...
            if (testError.code === 'ER_ACCESS_DENIED_ERROR') {
                console.log('\n⚠️  Password might be incorrect or user not properly created');
            }
            return false;
        }
...
verifyBackendUser().then(success => {
    if (success) {
        ...
        console.log('\n⚠️  Keep using avnadmin in .env until backend_app_user is fixed');
    }
